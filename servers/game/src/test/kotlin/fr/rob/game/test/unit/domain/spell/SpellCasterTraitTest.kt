...
import fr.rob.game.spell.SpellInfo.Companion.CASTING_TIME_2_SECONDS
import fr.rob.game.spell.effect.EffectFromSpellInterface
import fr.rob.game.spell.effect.EffectInterface
import fr.rob.game.spell.effect.InstantDamageEffect
...
import fr.rob.game.spell.type.projectile.GhostProjectileLaunchInfo
import fr.rob.game.spell.type.projectile.TimedProjectileLaunchInfo
...
class SpellCasterTraitTest : SpellCasterEnvironmentBaseTest() {
    ...
    fun `I should be able to cast spell with projectiles`() {
        ...
        caster.onUpdate(1000)
        assertEquals(99, target.getComponent<HealthComponent>()?.value)
    }
    ...
    fun `I should be able to cast spell with casting time`() {
        ...
        // 2 seconds casting
        caster.onUpdate(1000)
        assertEquals(94, target.getComponent<HealthComponent>()?.value)
    }
